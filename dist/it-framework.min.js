...
IT.Dialog = class extends IT.Component {
    constructor(t) {
        ...
        e.id = e.settings.id || IT.Utils.id();
        e.addEvents(e.settings, [ "onShow", "onHide", "onClose" ]);
    ...
    createElement() {
        ...
        $.each(t.settings.footers, function(e, n) {
            if (n) {
                if (!n.isClass) n = IT.Utils.createObject(n);
                if (n) n.renderTo(t.content.find(".it-dialog-footer")); else console.warn("Xtype: undefined", obj);
            }
        });
        ...
        t.content.find(".it-dialog-container").removeClass("dialog-show").one(transitionEnd, function() {
            t.content.removeClass("dialog-show").one(transitionEnd, function() {
                setTimeout(() => {
                    t.elExist = false;
                    t.content.remove();
                    t.doEvent("onClose", [ t, t.id ]);
                }, 700);
            });
        });
    }
    setScroll() {
        let t = this, e = t.content.find(".it-dialog-container");
        e.height($(window).height() <= t.content.find(".it-dialog-content").height() ? $(window).height() - 50 : "auto");
    }
};
...
IT.HTML = class extends IT.Component {
    constructor(t) {
        ...
        e.content.css(e.settings.css);
        if (e.settings.url) e.content.load(e.settings.url); else {
            var n = Object.keys(e.settings.data).length;
            if (n) {
                e.settings.content = e.templateReplacer(e.settings.content, e.settings.data);
            }
    ...
    setContent(t, e = false) {
        ...
        if (typeof t === "string") {
            if (Object.keys(this.settings.data).length) {
                t = me.templateReplacer(this.settings.content, this.settings.data);
            }
            this.content.append(t);
        } else t.appendTo(this.content);
    }
    templateReplacer(t, e) {
        return t.trim().replace(/\{\{([\w\.]*)\}\}/g, function(t, n) {
            var s = n.split("."), i = e[s.shift()];
            for (var a = 0, o = s.length; a < o; a++) i = i[s[a]];
