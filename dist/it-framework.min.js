...
!function(t) {
    ...
    constructor(t) {
        super(t);
        this._readyState = !1, this.addEvents(this.settings, [ "stateChange" ]), this.readyState = !0;
    }
    get readyState() {
    ...
    constructor(t) {
        super();
        let e = this;
        e.commited = !1, e.rawData = t, e.changed = {}, e.field = Object.keys(t), e.locked = [];
    }
    isChanged(t = null) {
    ...
    update(t, e) {
        ...
        if (n.rawData.hasOwnProperty(t)) {
            ...
            if (n.changed.hasOwnProperty(t)) return delete n.changed[t], !0;
    ...
    load(t = {}) {
        ...
        switch (e.settings.type) {
          ...
          case "array":
            ...
            void 0 === s || s ? void 0 !== e.settings.data ? ($.each(e.settings.data, (t, n) => {
                let s = new IT.RecordStore(n);
                s.commited = !0, e.data.push(s), e.total_rows++;
    ...
    replace(t = {}, e = 0) {
        let n = this;
        for (let s in t) n.data[e].update(s, t[s]);
    }
    indexOf(t = "", e) {
        let n = this;
        for (let s = 0; s < n.data.length; s++) if (n.data[s].rawData[t] === e) return s;
        return -1;
    }
    ...
    constructor(t) {
        ...
        e.createComponent(), !e.settings.store.isClass) {
            class t extends IT.Store {
                load(t) {
                    let n = !0, s = this;
                    s.doEvent("beforeLoad"), e.editors.forEach(t => {
            ...
            e.store = new t(e.settings.store), e.store.addEvents({
                beforeLoad: function() {
                ...
                afterLoad: function(t, n, s, i) {
                    ...
                    e.doEvent("onLoad", [ e, n ]), e.selectedRow = null, e.selectedColumn = null;
                },
            ...
            }), t = null, e.params = e.store.params;
        }
    }
    createComponent() {
        ...
        let n = $('<div class="it-datatable-wrapper"/>'), s = $('<div class="it-datatable-fixed-header"/>'), i = $("<table width='100%' />"), a = $("<thead />"), o = $("<tbody />");
        if (t.content.append(n.append(i.append(a))), e.customHeader) a.append($(e.customHeader)); else {
            let n, s = $("<tr/>");
            for (let i = 0; i < e.columns.length; i++) n = e.columns[i], s.append($("<th />", {
                css: {
                    "min-width": n.width,
                    width: n.width
                }
        ...
        if (e.enableFixedHeader && t.content.append(s.append(i.clone())), i.append(o), e.paging) {
    ...
    assignData(t) {
        ...
        let s = e.params.start, i = e.params.limit, a = s + i < t.total_rows ? s + i : t.total_rows, o = t.total_rows > 0 ? s + 1 + "/" + a : "0", l = Math.ceil(t.total_rows / i);
        ...
        e.content.find(".it-datatable-pagination-page").html(l), 0 == s && e.content.find(".it-datatable-pagination-current").val(1), 
        n.length) for (let t = 0; t < n.length; t++) e.addRow(n[t]);
    }
    renderTo(t) {
        super.renderTo(t);
        let e = this;
    ...
    loadPage(t) {
        let e = (t - 1) * this.paging.limit;
        this.content.find(".it-datatable-pagination .it-datatable-pagination-current").val(t), 
        this.store.load({
            params: {
                start: e,
    ...
    getSelectedRecords() {
        return this.selectedRow ? null : this.store.data[this.selectedRow];
    }
    addRow(t = {}) {
        let e = this, n = $("<tr>");
        for (let s = 0; s < e.settings.columns.length; s++) {
            let i = e.editors[s], a = e.settings.columns[s], o = a.dataIndex, l = t.get(o);
            i && (i = i.isClass ? i : IT.Utils.createObject($.extend(!0, {}, a.editor, {
                width: a.width
            })));
            ...
            i && "checkbox" == i.className && (c.find("div").empty(), i.addEvents({
                onChange: function() {
                    t.update(o, i.checked), c[t.isChanged(o) ? "addClass" : "removeClass"]("it-datatable-changed");
                }
            }), i.checked = !!l, i.renderTo(c.find("div"))), c.on("click", function() {
                e.selectedRow = c.parent().index(), e.selectedColumn = c.index(), e.content.find("tbody tr").removeClass("it-datatable-selected"), 
                c.parent().addClass("it-datatable-selected"), i && "checkbox" !== i.className && a.editor.editable && !c.hasClass("it-datatable-editing") && !t.isLocked(o) && (c.find("div").empty(), 
                c.addClass("it-datatable-editing"), i.val(t.getChanged(o) || t.get(o)), i.input.on("blur", function() {
                    i.validate() && (t.update(o, i.val()), i.input.off(), i.content.detach(), c.removeClass("it-datatable-editing"), 
                    c.find("div").html(IT.Utils.findData(t.getChanged(o) || t.get(o), d)), c[t.isChanged(o) ? "addClass" : "removeClass"]("it-datatable-changed"));
                }), i.renderTo(c.find("div")), i.input.focus());
            }), n.append(c);
        }
        e.content.find("tbody").append(n);
    }
    removeRow(t = -1) {
        t = t < 0 ? this.selectedRow : t, this.content.find("tbody>tr").eq(t).remove(), 
        this.selectedRow = null, this.selectedColumn = null;
    }
    ...
    constructor(t) {
        ...
        $.each(e.settings.items, function(t, n) {
            n && ("function" != typeof n.renderTo && (n = IT.Utils.createObject(n)), void 0 !== n.settings.flex && n.content.addClass("it-flex-item"), 
    ...
    constructor(t) {
        ...
        s.settings = $.extend(!0, {
            ...
            url: "",
            items: []
        }, t), s.addEvents(s.settings, [ "beforeSerialize", "beforeSubmit", "success", "error" ]), 
        s.id = s.settings.id || IT.Utils.id(), n = $("<div />", {
            id: s.id,
            class: "container-fluid"
        }), s.ids = [], s.items = {}, $.each(s.settings.items, function(t, i) {
            i && (i.isClass || (i = IT.Utils.createObject(i)), i.noRow ? i.renderTo(n) : (e = $("<div/>", {
                class: "row form-row"
            }), i.renderTo(e), n.append(e)), s.ids.push(i.getId()), s.items[i.getId()] = i);
        }), s.content = $("<form />", {
            name: IT.Utils.id(),
            class: "it-form",
            action: s.settings.url,
            target: "" | s.settings.target,
            enctype: "multipart/form-data"
        }), s.content.append(n), s.ajaxForm = s.content.ajaxForm($.extend({}, {
            url: s.content.prop("action"),
            type: "POST",
            dataType: "json",
            delegation: !0,
            beforeSerialize: function(t, e) {
                let n = s.doEvent("beforeSerialize", [ s, t, e ]);
                return void 0 === n || n;
            },
            beforeSubmit: function(t, e, n) {
                let i = s.doEvent("beforeSubmit", [ s, t, e, n ]);
                return void 0 === i || i;
            },
            success: function(t, e, n) {
                s.doEvent("success", [ t, s, n, e ]);
            },
            error: function(t, e, n) {
                s.doEvent("error", [ s, t, e, n ]);
            }
        }));
    }
    getItemCount() {
    ...
    setData(t) {
        let e = function(n) {
            $.each(n, function(n, s) {
                if ("function" == typeof s.val && "checkbox" != s.className && "radio" != s.className) {
                    let e = t.getChanged(s.settings.name) || t.get(s.settings.name);
                    s.val(e);
                } else s.items && e(s.items);
            });
        };
        e(this.items);
    }
    submit(t = null) {
        let e = this;
        null == t ? e.content.submit() : e.content.ajaxSubmit($.extend({
            url: e.content.prop("action"),
            type: "POST",
            dataType: "json",
            delegation: !0,
            beforeSerialize: function(t, n) {
                let s = e.doEvent("beforeSerialize", [ e, t, n ]);
                return void 0 === s || s;
            },
            beforeSubmit: function(t, n, s) {
                let i = e.doEvent("beforeSubmit", [ e, t, n, s ]);
                return void 0 === i || i;
            },
            success: function(t, n, s) {
                e.doEvent("success", [ t, e, s, n ]);
            },
            error: function(t, n, s) {
                e.doEvent("error", [ e, t, n, s ]);
            }
        }, t));
    }
    clear() {
    ...
    constructor(t) {
        ...
        e.settings = $.extend(!0, {
            ...
            css: {},
            items: []
        }, t), e.id = e.settings.id || IT.Utils.id(), e.settings.columnRule ? e.content = $("<div />", {
            id: e.id,
        ...
        }), "" == e.settings.columnRule && "fluid" == e.settings.rowContainer ? e.content = $("<div/>", {
            class: "container-fluid"
        }).append(e.content) : "" == e.settings.columnRule && "standar" == e.settings.rowContainer && (e.content = $("<div/>", {
            class: "container"
        }).append(e.content));
    }
    ...
    constructor(t) {
        ...
        e.settings.url) e.content.load(e.settings.url); else {
            Object.keys(e.settings.data).length && (e.settings.content = e.templateReplacer(e.settings.content, e.settings.data)), 
            e.content.html(e.settings.content);
        }
    }
    setContent(t, e = !1) {
        e && this.content.empty(), "string" == typeof t ? (Object.keys(this.settings.data).length && (t = me.templateReplacer(this.settings.content, this.settings.data)), 
        this.content.append(t)) : t.appendTo(this.content);
    }
    templateReplacer(t, e) {
        return t.trim().replace(/\{\{([\w\.]*)\}\}/g, function(t, n) {
            for (var s = n.split("."), i = e[s.shift()], a = 0, o = s.length; a < o; a++) i = i[s[a]];
    ...
    constructor(t) {
        ...
        }, t), e.id = e.settings.id || IT.Utils.id(), e.imagebox = '\n\t\t\t<div class="it-imagebox">\n\t\t\t\t<a href="javascript:void(0);" class="it-imagebox-chooser it-btn btn-primary">\n\t\t\t\t\t<span class="fa fa-picture-o"></span> &nbsp; Pilih Sumber Gambar\n\t\t\t\t</a>\n\t\t\t\t<input type="file" class="cropit-image-input">\n\t\t\t\t<div class="cropit-preview"></div>\n\t\t\t\t<div class="hide-this">\n\t\t\t\t\t<div class="image-size-label">Zoom</div>\n\t\t\t\t\t<input type="range" class="cropit-image-zoom-input" value="0">\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t', 
        e.reloadObject();
    }
    reloadObject() {
        var t = this;
        t.content = $(t.imagebox), t.content.find(".cropit-preview").width(t.settings.size.width).height(t.settings.size.height), 
        t.isCropper() ? (t.content.find(".hide-this").removeClass("it-hide"), t.content.find(".it-imagebox-chooser").click(function() {
            t.content.find(".cropit-image-input").click();
        }), t.content.cropit($.extend(!0, t.settings.cropperSettings)), "" != t.settings.src && t.content.cropit("imageSrc", t.settings.src)) : (t.content.find(".it-imagebox-chooser").hide(), 
        t.content.find(".hide-this").addClass("it-hide"), t.content.find(".cropit-preview").append($("<img/>", {
            class: "it-imagebox-picture",
            src: t.settings.src
        })));
    }
    renderTo(t) {
        super.renderTo(t), this._render = t;
    }
    isCropper() {
        return this.settings.cropper;
    }
    setIsCropper(t) {
        let e = this, n = e.settings.cropper;
        e.settings.cropper = t, n != e.settings.cropper && (e.content.remove(), e.reloadObject(), 
        e.renderTo(e._render));
    }
    setImageSrc(t) {
        let e = this;
        e.isCropper() ? e.content.cropit("imageSrc", t) : e.content.find("img").attr("src", t);
    }
    getExport() {
        let t = "This is not cropper.";
        return this.isCropper() && (t = this.content.cropit("export")), t;
    }
    ...
    constructor(t) {
        ...
        if (e.content = $(n), e.content.find(".it-messagebox-container").css($.extend(e.settings.css, {
            "max-width": e.settings.width
        })), 0 == e.settings.buttons.length) {
            new IT.Button({
                text: "OK",
                handler: function() {
                    e.hide();
                }
            }).renderTo(e.content.find(".it-messagebox-btn"));
        } else $.each(e.settings.buttons, function(t, n) {
    ...
    show() {
        $("input, select, textarea").blur(), this.content.show(0, function() {
    ...
    constructor(t) {
        ...
        e.content.last().append(e.input), e.settings.width && e.content.css({
            width: e.settings.width
        }), e.settings.withRowContainer && (e.content = $("<div/>", {
            class: "row"
        }).append(e.content)), e.addEvents(e.settings, [ "onLoad", "onChange" ]), e.input.on("change", function() {
    ...
    getDisplayValue() {
        return this.getSelect().getItem(this.val())[0].innerHTML;
    }
    setDataStore(t) {
    ...
    constructor(t) {
        ...
        }, t), e.id = e.s.id || IT.Utils.id(), e.readyState = !1, e.addEvents(e.s, [ "onLoad", "onChange", "onBlur", "query" ]), 
        ...
        e.searchInput = e.content.find("input.it-select-search"), e.dropContent = e.content.find("ul.it-select-dropdown"), 
        e.input = e.content.find("input.it-select-value"), e.s.emptyText && e.searchInput.attr("placeholder", e.s.emptyText), 
        e.s.width && e.content.css({
            width: e.s.width
        }), e.s.withRowContainer && (e.content = $("<div/>", {
            class: "row"
        }).append(e.content)), e.store = new IT.Store($.extend(!0, {}, e.s.store, {
            autoLoad: !1,
            beforeLoad: function() {
                e.readyState = !1;
            },
            afterLoad: function(t, n, s) {
                for (let t = 0; t < s.length; t++) e.addItem(s[t]), e.readyState = !0;
            },
            params: {
                q: ""
            }
            ...
            e.doEvent("onChange", [ e, e.val() ]);
        }), e.searchInput.on("focus", t => {
            e.dropContent.show();
        }).on("keyup", t => {
            "array" !== e.store.s.type && (e.dropContent.children().remove(), e.store.params.q = e.searchInput.val(), 
            e.getDataStore());
        }), e.s.autoLoad ? e.getDataStore() : e.readyState = !0;
    }
    getDataStore() {
        this.store.load();
    }
    renderTo(t) {
        super.renderTo(t), this.searchInput.css({
            width: this.s.width - this.dropContent.width()
        });
    }
    addItem(t) {
        let e = this, n = $("<li/>", {
            text: t.rawData.value,
            attr: {
                "data-value": t.rawData.key
            }
        }).on("click", t => {
            n.parent().find("li").removeClass("it-select-selected"), n.addClass("it-select-selected"), 
            e.searchInput.val(n.text()).blur(), e.input.val(n.data("value")).change(), e.dropContent.hide();
        });
        e.dropContent.append(n);
    }
    val(t) {
        return void 0 === t ? this.input.val() : this.input.val(t);
    }
    ...
    constructor(t) {
        ...
        e.settings = $.extend(!0, {
            ...
            width: e.settings.width
        }), e.settings.style && e.content.css(e.settings.style), e.addEvents(e.settings, [ "onLoad", "onChange" ]), 
    ...
    setActive(t) {
        let e, n = this.content.find(".it-tabs-menu li").eq(t);
        if (n.length < 1) throw "offset index";
        this.content.find(".tab-active").removeClass("tab-active"), e = n.addClass("tab-active"), 
        this.content.find("#" + e.data("tab")).addClass("tab-active");
    }
    getActive() {
    ...
    constructor(t) {
        ...
        }, t), e = n.settings, n.id = e.id || IT.Utils.id(), "" == e.label && (e.size.field = "col"), 
        e.type) {
          case "textarea":
          ...
          case "hidden":
            n.input = $(`<input id="${n.id}-item" ` + "type='hidden' " + `name='${n.settings.name || IT.Utils.id()}' ` + `${e.value ? `value='${e.value}'` : ""} ` + ">");
            break;
        ...
        e.info.prepend && s.prepend($("<div />", {
            ...
            class: "it-edit-item",
            html: e.info.append
        })), n.content = $((e.label ? `<div class="${e.size.label}">\n\t\t\t<label for="${n.id}-item" class='it-input-label it-input-label-${e.labelAlign || "left"}'> ${e.label} </label>\n\t\t</div>` : "") + `<div class="${e.size.field}"></div>`), 
        n.content.last().append(s), e.withRowContainer && (n.content = $("<div/>", {
            class: "row"
        }).append(n.content)), n.readyState = !0;
    }
    ...
    static findData(t, e, n = null) {
        ...
        if (n = $.extend(!0, {
            field: "key",
            look: "value"
        }, n || {}), i.length) for (let e = 0; e < i.length; e++) {
            let a = i[e];
            if (a[n.field] == t) {
                s = a[n.look];
                break;
