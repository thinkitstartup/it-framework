...
!function(t) {
    ...
    constructor(t) {
        ...
        n.input = $(`<input id="${n.id}-item" ` + `type='checkbox' ` + `name='${e.name || IT.Utils.id()}' ` + `${0 == e.allowBlank ? `required` : ""} ` + `${e.readonly ? ` readonly ` : ""} ` + `${0 == e.enabled ? ` disabled ` : ""} ` + `value='${e.value || IT.Utils.id()}' ` + `>`), 
        n.input.on("change", function() {
            n.doEvent("onChange", [ n, this.checked ]);
        }), n.content = $("<div class='it-edit for-option' />").append(n.input).append(`<label for="${n.id}-item" ` + `>${e.label}</label>`), 
        n.readyState = !0;
    }
    get checked() {
    ...
    createComponent() {
        ...
        if (t.content.append(n.append(s.append(i))), e.customHeader) i.append($(e.customHeader)); else {
            ...
            }).append(n.header)), t.editors.push(n.editor ? IT.Utils.createObject($.extend(!0, {}, n.editor, {
                width: n.width
            })) : null);
            i.append(a);
        }
        ...
        n.length) for (let t = 0; t < n.length; t++) {
            ...
            for (let n = 0; n < e.settings.columns.length; n++) {
                ...
                g && (c.on("click", function() {
                    ...
                    c.parent().addClass("it-datatable-selected"), i.editor.editable && !$(this).hasClass("it-datatable-editing") && (console.info("set "), 
                    c.find("div").empty(), c.addClass("it-datatable-editing"), g.val(a.getChanged(o) || a.get(o)), 
