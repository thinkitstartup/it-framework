...
IT.BaseClass = class {
    ...
    constructor(t) {
        ...
        e.params = {}, e.selectedRow = null, e.selectedColumn = null, e.paging = {
            page: 1,
            page_count: 0,
            total_rows: 0
        }, e.createComponent(), e.settings.store.isClass || (e.store = new IT.Store($.extend(!0, {
            beforeLoad: function() {
                console.info("load"), e.content.find(".it-datatable-wrapper").animate({
                    scrollTop: 0
                }, "slow"), e.content.find(".it-datatable-loading-overlay").addClass("loading-show");
            },
            afterLoad: function(t, n, s) {
                e.assignData(t), e.listener.fire("onLoad", [ e, t ]), e.content.find(".it-datatable-loading-overlay").removeClass("loading-show");
            },
            onEmpty: function(t, n, s) {
                e.assignData(t), e.listener.fire("onLoad", [ e, t ]), e.content.find(".it-datatable-loading-overlay").removeClass("loading-show");
            }
        }, e.settings.store)), e.params = e.store.params);
    }
    createComponent() {
        ...
        let n = $(`<div class="it-datatable-wrapper"/>`), s = $(`<div class="it-datatable-fixed-header"/>`), a = $(`<table width='100%' />`), i = $(`<thead />`), o = $(`<tbody />`);
        if (t.content.append(n.append(a.append(i))), e.customHeader) i.append($(e.customHeader)); else {
            let t, n = $(`<tr/>`);
        ...
        if (e.enableFixedHeader && t.content.append(s.append(a.clone())), a.append(o), e.paging) {
    ...
    assignData(t) {
        ...
        let s = e.params.start, a = e.params.limit, i = s + a < t.total_rows ? s + a : t.total_rows, o = t.total_rows > 0 ? s + 1 + "/" + i : "0", l = Math.ceil(t.total_rows / a);
        ...
        n.length) for (let t = 0; t < n.length; t++) {
            ...
            for (let n = 0; n < e.settings.columns.length; n++) {
                let i = e.settings.columns[n], o = e.settings.columns[n].dataIndex, l = s.get(o);
                l = l || "";
                let d, r = $("<td />", {
                    html: $("<div />", {
                        html: l
                ...
                r.on("click", function() {
                    ...
                    r.parent().addClass("it-datatable-selected"), i.editor && i.editor.editable && !r.hasClass("it-datatable-editing") && (r.addClass("it-datatable-editing"), 
                    r.attr("data-oldval", l), (d = IT.Utils.createObject(i.editor)).val(r.find("div").html()), 
                    r.find("div").empty(), d.input.on("blur", function() {
                        d.validate() && (s.update(o, d.val()), r.removeClass("it-datatable-editing"), r.find("div").html(d.val()), 
                        d.content.remove(), r[d.val() == r.data("oldval") ? "removeClass" : "addClass"]("it-datatable-changed"));
                    }), d.renderTo(r.find("div")), d.input.focus());
                }), a.append(r);
            }
            e.content.find("tbody").append(a);
        }
    }
    renderTo(t) {
        super.renderTo(t);
    ...
    constructor(t) {
        ...
        $.each(e.settings.items, function(t, e) {
            e && (n = $("<div>", {
                class: "row"
            }), e.isClass || (e = IT.Utils.createObject(e)), e.renderTo(n), s.append(n), a++);
        }), e.content = $("<form />", {
            name: IT.Utils.id(),
            class: "it-form"
        }), e.content.append(s);
    ...
    getChanged() {
        let t = this;
        return t.isChanged() ? Object.assign({}, t.rawData, t.changed) : null;
    }
    get(t) {
    ...
    constructor(t) {
        ...
        }, t), e.id = e.settings.id || IT.Utils.id(), e.select = $("<select />", {
            id: e.id,
            name: e.id,
            attr: {
    ...
    constructor(t) {
        ...
        }, t), e.params = e.settings.params, e.data = [], e.total_rows = 0, e.listener = new IT.Listener(e, e.settings, [ "beforeLoad", "afterLoad", "onLoad", "onError", "onEmpty" ]), 
        e.settings.autoLoad && e.load();
    }
    empty() {
        let t = this;
        t.total_rows = 0, t.data = [], t.listener.fire("onEmpty", [ t, t.getData(), t.params ]);
    }
    load(t = {}) {
        let e = this;
          ...
          case "json":
            ...
            e.params = n, e.empty(), $.ajax({
                ...
                beforeSend: function(t, n) {
                    return e.total_rows = 0, e.listener.fire("beforeLoad", [ e, t, n ]);
                },
                success: function(t) {
                    void 0 !== t.rows && void 0 !== t.total_rows ? ($.each(t.rows, (t, n) => {
                        e.data.push(new IT.RecordStore(n));
                    }), e.total_rows = t.total_rows, e.listener.fire("onLoad", [ e, e.getData(), e.params ])) : (e.empty(), 
                ...
                complete: function() {
                    e.listener.fire("afterLoad", [ e, e.getData() ]);
                }
            });
            break;

          case "array":
            e.total_rows = 0, !e.beforeLoad || e.beforeLoad && e.listener.fire("beforeLoad", [ e, e.data || [], null ]) ? (void 0 !== e.settings.data ? ($.each(e.settings.data, (t, n) => {
                e.data.push(new IT.RecordStore(n)), e.total_rows++;
            }), e.listener.fire("onLoad", [ e, e.getData(), null ])) : e.listener.fire("onError", [ e, {
                status: !1,
                message: "Data JSON '" + e.settings.url + "' Tidak Ditemukan"
            } ]), e.listener.fire("afterLoad", [ e, e.getData() ])) : (e.empty(), e.listener.fire("onError", [ e, {
                status: !1,
                message: "Format Data Tidak Sesuai"
            } ]));
        }
    ...
    setData(t) {
        ...
        t = "json" == e.type ? t.rows : t, e.empty(), $.each(t, (t, n) => {
            e.data.push(new IT.RecordStore(n)), e.total_rows++;
        }), e.listener.fire("onLoad", [ e, e.data, e.params ]);
    }
    getSetting() {
        return this.settings;
    }
    ...
    constructor(t) {
        ...
        $.each(e.settings.titles.items, function(t, n) {
            ...
            a.appendTo(e.content.find(".it-tabs-menu")), a.click(function() {
